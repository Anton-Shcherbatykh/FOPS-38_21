...
Создаю манфиест [sc.yaml](https://github.com/Anton-Shcherbatykh/FOPS-38_21/blob/main/21-05/Files/sc.yaml). 

P.S. в предоставленно шаблоне манифеста указан ```provisioner: kubernetes.io/no-provisioner```, что означает отсутствие динамического провижионирования. То есть при создании PVC не будет автоматически создан PV. Следовательно, PV нужно создавать вручную — либо отдельным манифестом, либо ожидать, что кто-то другой его предоставит. В своём решении я добавил явное создание PV (```manual-pv-for-sc```), чтобы PVC мог привязаться. Однако, это выходит за рамки задания «создать SC и PVC» — я создал ещё и PV, что не было явно указано в задании, но технически необходимо для работы. 
